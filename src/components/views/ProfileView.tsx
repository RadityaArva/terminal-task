...
export default function ProfileView() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 font-mono"><p className="text-xs text-[var(--text-muted)]"><span className="text-[var(--accent-main)] font-bold">$</span> whoami --profile <span className="text-[var(--accent-cyan)]">· {profile.username}</span></p>
      <section className="terminal-panel flex flex-wrap items-center gap-5 p-5 sm:p-6">
        <div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-full border-2 border-[var(--accent-cyan)] bg-[var(--accent-purple)] text-3xl font-bold text-[var(--bg-app)]">
          {profile.avatarUrl ? <img src={profile.avatarUrl} alt={profile.name} className="img-fit" loading="lazy" /> : profile.name.charAt(0)}
        </div>
        ...
        <div className="flex gap-2"><button onClick={() => setModal('profile')} className="terminal-button"><SquarePen size={13} className="mr-1.5 inline" aria-hidden /> Edit profile</button><button onClick={() => setModal('settings')} className="terminal-button"><Settings size={13} className="mr-1.5 inline" aria-hidden /> Settings</button></div>
...
function ProfileModal({ profile, onClose, onSave }: { profile: UserProfile; onClose: () => void; onSave: (updates: Partial<UserProfile>) => void }) {
  ...
  return <ModalShell title="profile --edit" onClose={onClose}><form onSubmit={submit} className="grid gap-3 text-xs"><div className="flex items-center gap-3 rounded border border-[var(--border-main)] bg-[var(--bg-app)] p-3"><div className="h-12 w-12 shrink-0 overflow-hidden rounded-full border border-[var(--border-main)] bg-[var(--bg-surface)]">{draft.avatarUrl ? <img src={draft.avatarUrl} alt="Preview" className="img-fit" loading="lazy" /> : <span className="flex h-full w-full items-center justify-center font-bold">{draft.name.charAt(0)}</span>}</div> <div className="min-w-0"><span className="block text-[var(--text-muted)]">Profile identity</span><label className="mt-2 inline-flex cursor-pointer items-center rounded border border-[var(--accent-cyan)]/60 px-2 py-1 text-[11px] font-bold text-[var(--accent-cyan)] transition hover:bg-[var(--accent-cyan)]/10"><span><ImagePlus size={13} className="mr-1.5 inline" aria-hidden /> Pilih foto</span><input type="file" accept="image/*" className="sr-only" onChange={choosePhoto} /></label><span className="ml-2 text-[10px] text-[var(--text-muted)]">JPG, PNG, WEBP · maks. 2 MB</span>{photoError && <p className="mt-1 text-[10px] text-[var(--accent-red)]">{photoError}</p>}</div></div><input className="terminal-input" value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} placeholder="Nama" /><input className="terminal-input" value={draft.username} onChange={(event) => setDraft({ ...draft, username: event.target.value })} placeholder="Username" /><input className="terminal-input" value={draft.avatarUrl} onChange={(event) => { setDraft({ ...draft, avatarUrl: event.target.value }); setPhotoError(''); }} placeholder="URL foto profile (opsional)" /><input className="terminal-input" value={draft.role} onChange={(event) => setDraft({ ...draft, role: event.target.value })} placeholder="Role" /><textarea className="terminal-input" value={draft.bio} onChange={(event) => setDraft({ ...draft, bio: event.target.value })} placeholder="Bio" /><button className="terminal-button terminal-button-primary">Simpan profile</button></form></ModalShell>;
}
