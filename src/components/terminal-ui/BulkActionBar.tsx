...
export default function BulkActionBar({ ids, onClear }: { ids: string[]; onClear: () => void }) {
  ...
  return (
    <div className="sticky bottom-3 z-20 flex flex-wrap items-center gap-2 rounded-lg border border-[var(--accent-cyan)] bg-[var(--bg-surface)] p-3 text-xs shadow-xl">
      <strong className="mr-auto text-[var(--accent-cyan)]">{ids.length} selected</strong>
      ...
      <button type="button" onClick={() => { if (confirm(`Delete ${ids.length} tasks?`)) { bulkDeleteTasks(ids); onClear(); } }} className="terminal-button border-[var(--accent-red)] text-[var(--accent-red)]">Delete</button>
      <button type="button" onClick={onClear} className="terminal-button">Clear</button>
    </div>
  );
}
